...
//===========================时钟初始化函数=========================================//
	void Init_clock(void)
	{
  	...
  	_wdtc=0x57;            //使能看门狗
  	_sadc0=0x23;           //选择通道AN3
  	_sadc1=0x03;           //外部模拟通道输入，时钟源fsys/2
  	_sadc2=0x00;           //内部VDD作为参考电压
  	_acerl=0x08;		       //A/D输入AN3
...
u16 Get_AD()
{
	u8 i,j,AD_Count;
	u16 AD;
	for(AD_Count=0;AD_Count<10;AD_Count++)
	{	
	  	_start=0; 
	    _start=1;  
	    _start=0;				//启动AD转换	          
	    ...
	    //ADH= _sadoh;
	    //ADH=ADH<<8;
	    //AD_Mun[AD_Count]=ADH+ADL;	 //处理AD的结果	     
      ADL= _sadol;
	    ADH= _sadoh;
	    AD_Mun[AD_Count]=((u16)ADH<<4) + ((ADL>>4)&0x0f);	 //处理AD的结果   	   
	}  	  	               	    
  for(j=1;j<10;j++)   		//从小到大排序 冒泡排序
  { 
    for(i=0;i<10-j;i++)  
    {
	    if(AD_Mun[i]>AD_Mun[i+1])
	    {
		    temp=AD_Mun[i];
		    AD_Mun[i]=AD_Mun[i+1];
		    AD_Mun[i+1]=temp;
...
//=====================数码管更新显示=========================//	
      ...
      {	
       t10msflag=0;	  	
			...
			{
				AD_Last = AD_NEW;
				AD_Value=AD_Last*5/4095;
				Voltage=(u16)(AD_Value*100);
				num1=Voltage/100;
				num2=Voltage/10%10;
				num3=Voltage%10;
				num4=10;				
